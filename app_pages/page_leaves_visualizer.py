...
def page_leaves_visualizer_body():
    ...
    st.info(
        ...
        f"differentiates a leaf that contains mildew from an healthy leaf.")
    
    version = 'v1'
    if st.checkbox("Difference between average and variability image"):
      
      avg_mildew = plt.imread(f"output/{version}/avg_var_powdery_mildew.png")
      avg_healthy = plt.imread(f"output/{version}/avg_var_healthy.png")
    ...
    if st.checkbox("Differences between average mildew containing leaf and average healthy leaf"):
          diff_between_avgs = plt.imread(f"output/{version}/avg_diff.png")

          st.warning(
            f"* We notice this study didn't show any significant"
            f"patterns where we could intuitively differentiate one from another except a samll color difference.")
